...
We'll use a Seurat object distributed via `SeuratDisk`:
...
```{r}
hdf5r::h5ls("cbmc.h5mu", recursive = FALSE)
```
...
```{r}
head(cbmc_u[["RNA"]]@features.metadata[,1:4])
head(cbmc_r[["RNA"]]@features.metadata[,1:4])
```
